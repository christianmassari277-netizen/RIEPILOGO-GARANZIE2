...
def _read_text(path):
    # Prova prima latin-1 (tipico dei file legacy); fallback a utf-8
    for enc in ("latin-1", "utf-8", "cp1252"):
        try:
            ...
            continue
    # Ultimo tentativo senza encoding
...
def processa_file(input_path, output_path):
    ...
    # Regex: 7 cifre (NUMERO GARANZIA), poi SUFFISSO, JOB e TOTALE JOB come interi
    # Consente spazi multipli; ignora righe non conformi
    ...
    if not records:
        # Crea un PDF minimale che spiega il problema
        doc = SimpleDocTemplate(output_path, pagesize=A4)
        styles = getSampleStyleSheet()
        elements = [Paragraph("Nessun dato riconosciuto", styles["Title"]),
...
def main():
    if len(sys.argv) >= 2:
        ...
        # Se l'utente apre l'exe senza parametri, mostra un messaggio in finestra (se su Windows)
        try:
            import ctypes
